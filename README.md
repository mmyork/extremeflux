...
- Initial models were run to classify strong sink, strong source, and inactivity at each site. Seven sites were used (SES,SEG,MPJ,WJS,VCP,VCS,SR), so 21 models ran overall. Only NEE was used to determine strong sink (< 5th percentile), inactive (around 0), and strong source (> 95th percentile). Grouped variable importances were ran as well across variable and timescale groupings.
- To combat very high inactive state misclassifications, we tried running the inactive states in two seperate models: one with negative NEE points only and the other with positive NEE points only. This didn't help the misclassifications, but it did start us on the train of thought regarding confounding processes inside and outside of the growing season.
- Growing season was introduced. Data was split into growing and not growing season based on a 7 week moving lag (I'm not sure what the GPP threshold was here though). Models were run on each of the 3 states inside and outside of the growing season for each site seperately (3 states x 2 seasons x 7 sites = 42 models).
- Growing season was changed to be based on a historical average, and all models were rerun. This is not the same as the current historical average growing season algorithm being used because there could be multiple seasons each year. After this, the historical definition was tabled and we reverted to the moving lag average definition for a while.
- Definitions of all the extreme states were refined. Now, strong source = high reco + low gpp and strong sink = low reco + high gpp. The inactive definition was changed to require a low GPP value in order to prevent days where plant activity was high but cancelled out by high Reco being included as inactive. Reran all 42 models with the refined definitions.
- In addition to the models above, GPP and Reco were classified into low (<5th percentile), high (>95th percentile), and typical (all other) states. NEE states were removed from the covariates list, and RF models were run to classify GPP and Reco (low and high) with the remaining climatic variables. This was ran on all data and not the separate seasonal periods.
- The 7 sites were combined into one dataset and the same models (3 state x (in season + out of season + overall) = 9 models) were ran over all sites at once. No growing season split was run for these. Site ID was included as a variable as well. This is when we noticed that lagged NEE (previous NEE states) variables were soaking up all the information and importance from climate variables, as they were still included in the model. 
- Previous NEE state variables were removed, and the otherwise exact same models from the most previous step were all run again. Grouped variable importances were also ran on this.
- Fluxnet 2015 data ws obtained and models were rerun across the now 57ish sites. This dataset doesn't include SWC, so they were initially run without it. We are still waiting on getting that data to add!
