...
vnamessink <- colnames(bindat.in)[c(43,11,7:10,14:41)]
print("good var")

# run NEE models
set.seed(76786+12366)

sink.in <- imbalanced.rfsrc(sink ~.,data=bindat.in[vnamessink], na.action="na.omit",
                             method="rfq",importance="permute",block.size=1,
                             perf.type="misclass")
saveRDS(sink.in,file="sink.in.results.rds")