...
vnamessource <- colnames(bindat.all)[c(44,11,7:10,14:41)]
print("good var")

# run NEE models
set.seed(76786+12366)

source.all <- imbalanced.rfsrc(source ~.,data=bindat.all[vnamessource], na.action="na.omit",
                             method="rfq",importance="permute",block.size=1,
                             perf.type="misclass")
save(source.all,file="./results/source.all.R")
e.all,file="./results/source.all.R")
